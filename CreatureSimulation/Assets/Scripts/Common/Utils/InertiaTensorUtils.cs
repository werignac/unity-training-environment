...
public static class InertiaTensorUtils
{
	...
	// Added by William
	public static Matrix4x4 CalculateInertiaTensorMatrix(ArticulationBody ab)
	{
		return CalculateInertiaTensorMatrix(ab.inertiaTensor, ab.inertiaTensorRotation);
	}

	// Added by William https://forum.unity.com/threads/inertia-tensor-in-matrix-form-from-inertiatensor-and-inertiatensorrotation.940377/#:~:text=If%20you%20want%20to%20use%20it%20in%20world%20reference
	public static Matrix4x4 CalculateWorldInertialTensorMatrix(ArticulationBody ab)
	{
		Matrix4x4 rotationMatrix = Matrix4x4.Rotate(ab.transform.rotation);
		return rotationMatrix * CalculateInertiaTensorMatrix(ab) * rotationMatrix.transpose;
	}
