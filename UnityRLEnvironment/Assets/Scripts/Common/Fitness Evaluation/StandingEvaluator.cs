...
namespace werignac.RLEnvironment
{
	public class StandingEvaluator : MonoBehaviour, IFitnessEvaluator
	{
		...
		private void EvaluateVelocity(GameObject bodyPart)
		{
			...
			float linearVelocity = body.velocity.magnitude;
			float angularVelocity = body.angularVelocity.magnitude;

			float deltaScore = (0.5f - linearVelocity) + (0.5f - angularVelocity);

#if UNITY_EDITOR
			if (debug)
				Debug.LogFormat("Momentum Body Part: {0}\n\tLinear Momentum: {1}\n\tAngular Momentum: {2}\n\tDelta Score: {3}", bodyPart.name, linearVelocity, angularVelocity, deltaScore);
#endif
