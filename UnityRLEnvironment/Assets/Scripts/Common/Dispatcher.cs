...
using werignac.RLEnvironment.Settings;
...
namespace werignac.RLEnvironment.Dispatch
{
	/// <summary>
	...
	public class Dispatcher : MonoBehaviour
	{
		...
		private void ReadyExperiment(bool addListeners)
		{
			...
			if (addListeners)
			{
				experiment.GetOnCreatureStartedEvent().AddListener(OnSessionStartedSimulation);
				experiment.GetOnCreatureScoredEvent().AddListener(OnSessionFinishedSimulation);
				experiment.GetOnExperimentTerminatedEvent().AddListener(OnAllSimulationsFinished);
			}

			Communicator.Write("SUCCESS");

			experiment.StartReadSessionInitializationData();
		...
		private void OnSessionStartedSimulation(SimulationInitializationData sessionInitData)
		{
			Debug.LogFormat("Creature {0} started simulation.", sessionInitData.Index);
			if (Communicator != null)
			{
				Debug.Log($"Writing \"{sessionInitData.Index}\" to pipe.");
				Communicator.Write(sessionInitData.Index.ToString());
			}
		}
		...
		private void OnSessionFinishedSimulation(SimulationInitializationData sessionInitData, float score, string toSend)
		{
