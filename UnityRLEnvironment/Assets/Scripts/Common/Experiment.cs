...
namespace werignac.RLEnvironment
{
	...
	{
		public void StartReadSessionInitializationData();
	...
	public abstract class Experiment<Init_Type, Random_Init_Type, Serializable_Init_Type> : MonoBehaviour, IExperiment where Init_Type : SimulationInitializationData where Random_Init_Type : RandomSimulationInitializationDataFactory<Init_Type>, new() where Serializable_Init_Type : new()
	{
		...
		/// <summary>
		/// Communicator to external sources.
		/// </summary>
		PipeCommunicatorBuffer communicatorBuffer;
		...
		public UnityEvent<SimulationInitializationData, float, string> OnSessionScored = new UnityEvent<SimulationInitializationData, float, string>();
		...
		private void Start()
		{
			if (readCreatureOnStart)
				StartReadSessionInitializationData();
		}
#endif
		...
		private void Initialize()
		{
			...
			// Listen to events.
			populationController.onSessionFinishedSimulation.AddListener(OnSessionFinishedSimulation);
			populationController.onOutOfSimulationSessionInitData.AddListener(CheckOutOfAllCreatures);
			populationController.onSessionStartedSimulation.AddListener(OnCreatureStartedListener);
		...
		public void StartReadSessionInitializationData()
		{
			...
			if (dispatcher != null)
			{
				...
				communicatorBuffer = dispatcher.CommunicatorBuffer;
			}
			else
			{
		...
		/// </summary>
		/// <param name="sessionInitData"></param>
		/// <param name="score"></param>
		private void OnSessionFinishedSimulation(Init_Type sessionInitData, float score)
		{
			Debug.LogFormat("Creature {0} finished with score: {1}", sessionInitData.Index, score);
			string toSend = $"{sessionInitData.Index} {score}";
			OnSessionScored.Invoke(sessionInitData, score, toSend);
		}
		...
		{
			if (!(jsonParser == null && populationController.IsOutOfSimulationSessionInitData()))
				return;
